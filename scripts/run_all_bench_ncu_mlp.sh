...
TARGET_LAYER_NVTX_MARKER="layer=18_module=MLP"
TARGET_NAME="mlp"
...
LOG_FILE="${LOG_DIR}/run_all_bench_ncu_${TARGET_NAME}_prefill_decode_${timestamp}_mlp.log"
...
for phase_mode in "${NCU_PHASE_MODES[@]}"; do
	...
	for batch_size in "${BATCH_SIZES[@]}"; do
		for prompt_length in "${PROMPT_LENGTHS[@]}"; do
			...
			report_prefix="${RUN_REPORT_DIR}/benchmark_${TARGET_NAME}_bs${batch_size}_pl${prompt_length}_ws${WARMUP_ITERATIONS}_${phase_mode}_phase_mlp"
